...
export default function Continue() {
    ...
    useEffect(() => {
        ...
        if (raw) {
            try {
                ...
                console.error("Failed to parse movie continue list:", e);
            }
        }
        setIsLoaded(true);
    }, [session]);
    ...
    const handleRemoveItem = (itemId) => {
        ...
        localStorage.setItem("cinestream_movies_continue", JSON.stringify(updatedList));
    };
                    ...
                    <div className="relative aspect-video rounded-xl border border-dashed border-white/10 bg-white/[0.01] backdrop-blur-md p-5 flex flex-col justify-between shadow-md group hover:border-primary/25 transition-all duration-300 select-none">
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-xs text-on-surface-variant leading-relaxed">
                                Start playing movies and they'll show up here automatically.
                            </p>
                        </div>
                    </div>

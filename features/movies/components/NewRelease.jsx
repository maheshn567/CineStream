...
export default function NewRelease() {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ["new-release", id],
        queryFn: () => {
            if (id) {
                return fetch(`${process.env.NEXT_PUBLIC_TMDB_BASE_URL}/3/discover/movie?with_genres=${id}`, {
                    headers: {
            ...
            return fetch(`${process.env.NEXT_PUBLIC_TMDB_BASE_URL}/3/movie/now_playing`, {
                headers: {
                    accept: 'application/json',
                    Authorization: `Bearer ${process.env.NEXT_PUBLIC_ACCESS_TOKEN}`
                }
    ...
    if (isLoading) {
        return (
            <section className="bg-surface py-8 px-gutter max-w-container-max mx-auto w-full space-y-md">
                ...
                {id ? (
                    /* Grid Skeleton Loader for Genre Browse view */
                    ...
                    /* Scroll Row Skeleton Loader for Home view */
        ...
        <section className="bg-surface py-8 px-gutter max-w-container-max mx-auto w-full space-y-md">
            ...
            <div className="flex justify-between items-end border-l-4 border-primary-container pl-4 mb-6">
                <h2 className="font-headline-md text-headline-md text-on-surface uppercase font-bold tracking-tight">
                    {id ? "Genre Spotlight" : "New Releases"}
                </h2>
            </div>

            {id ? (
                /* Grid Layout for Genre Browse view */
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-md">
                    {movies.map((item) => (
                ...
                /* Horizontal Scroll layout for Home Feed view */
                <div className="relative group/nav">
                    ...
                    {/* Right spacer to prevent edge clipping */}

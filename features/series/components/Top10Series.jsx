...
export default function Top10Series() {
    ...
    const { data, isLoading, error } = useQuery({
        ...
        queryFn: () => {
            return fetch(`${process.env.NEXT_PUBLIC_TMDB_BASE_URL}/3/tv/top_rated?language=en-US&page=1`, {
                headers: {
        ...
        return (
            <section className="bg-surface py-8 px-gutter max-w-container-max mx-auto w-full">
                <div className="border border-error/20 bg-error/5 text-error px-6 py-4 rounded-xl text-body-md font-semibold text-center">
                    Error loading top rated series: {error.message}
                </div>
            </section>
        );
    }

    // Showcase the top 10 series
